...
#!/bin/bash
HOST='146.190.80.32'
USER='sql_aureamanila_'
PASS='xhsbSN6CAEWamGDn'
DB='sql_aureamanila_'
EOM7
...
EOM
wget -O /usr/local/sbin/ssh.php https://aureamanila.online/enseivpn_api/prem -q
}
...
[ "$user_name" != '' ] && [ "$user_name" = "$username" ] && echo "user : $username" && echo 'authentication ok.' && exit 0 || echo 'authentication failed.'; exit 1
EOM
wget -O /usr/local/sbin/ssh.php https://aureamanila.online/enseivpn_api/vip -q
}
...
[ "$user_name" != '' ] && [ "$user_name" = "$username" ] && echo "user : $username" && echo 'authentication ok.' && exit 0 || echo 'authentication failed.'; exit 1
EOM
wget -O /usr/local/sbin/ssh.php https://aureamanila.online/enseivpn_api/private -q
}
